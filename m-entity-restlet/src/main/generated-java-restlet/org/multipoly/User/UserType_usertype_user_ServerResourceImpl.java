...
import org.multipoly.User.UserType.UserTypeRuntimePropertyEnum;
...
public class UserType_usertype_user_ServerResourceImpl extends ServerResource {
	private Object usertypeId;

	/**
	 * default constructor for UserType_usertype_user_ServerResourceImpl
	 */
	public UserType_usertype_user_ServerResourceImpl()  {
		setNegotiated(false);
	}
	...
	public Representation get() throws ResourceException {
		try {
			this.usertypeId = UmlgURLDecoder.decode((String)getRequestAttributes().get("usertypeId"));
			UserType parentResource = UMLG.get().getEntity(usertypeId);
			...
			json.append("],");
			json.append(" \"meta\" : {");
			json.append("\"qualifiedName\": \"RootElement::org::multipoly::User::UserType::user\"");
	...
	public Representation options() throws ResourceException {
		try {
			...
			json.append(", \"from\": ");
			json.append(UserTypeRuntimePropertyEnum.asJson());
			json.append("}}");
			json.append("]");
			return new JsonRepresentation(json.toString());
		} finally {
	...
	public Representation post(Representation entity) throws ResourceException {
		...
		try {
			...
			for ( Class<? extends User> baseClass : resultMap.keySet() ) {
				...
				result.append("],");
				result.append(" \"meta\" : {");
				result.append("\"qualifiedName\": \"RootElement::org::multipoly::User::UserType::user\"");
				result.append(", \"to\": ");
				if ( baseClass.equals(User.class) ) {
					result.append(UserRuntimePropertyEnum.asJson());
					result.append(", \"from\": ");
					result.append(UserTypeRuntimePropertyEnum.asJson());
					result.append("}");
					result.append("");
				} else {
	...
	private void add(Map<Class<? extends User>,List<UmlgNodeJsonHolder>> resultMap, UserType parentResource, Map<String,Object> propertyMap) {
	...
	private void delete(UserType parentResource, Map<String,Object> propertyMap) {
		Object id = propertyMap.get("id");
		User childResource = UMLG.get().getEntity(id);
