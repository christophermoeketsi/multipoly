...
public class User_user_usergroup_ServerResourceImpl extends ServerResource {
	private Object userId;

	/**
	 * default constructor for User_user_usergroup_ServerResourceImpl
	 */
	public User_user_usergroup_ServerResourceImpl()  {
		setNegotiated(false);
	}
	...
	public Representation get() throws ResourceException {
		try {
			...
			json.append("[");
			json.append("{\"data\": ");
			if ( parentResource.getUsergroup() != null && parentResource.getUsergroup().getClass() == UserGroup.class ) {
				json.append(ToJsonUtil.toJsonWithoutCompositeParent(parentResource.getUsergroup()));
			} else {
				json.append("null");
			}
			json.append(",");
			json.append(" \"meta\" : {");
			json.append("\"qualifiedName\": \"RootElement::org::multipoly::User::User::usergroup\"");
			json.append(",\"qualifiedNameFrom\": \"" + parentResource.getQualifiedName() + "\"");
			json.append(",\"qualifiedNameTo\": \"RootElement::org::multipoly::User::UserGroup\"");
			json.append("}");
			json.append("}]");
	...
	public Representation post(Representation entity) throws ResourceException {
		...
		try {
			...
			Map<Class<? extends UserGroup>,List<UmlgNodeJsonHolder>> resultMap = new HashMap<Class<? extends UserGroup>, List<UmlgNodeJsonHolder>>();
			...
			int count = 1;
			for ( Class<? extends UserGroup> baseClass : resultMap.keySet() ) {
				...
				result.append(",");
				result.append(" \"meta\" : {");
				result.append("\"qualifiedName\": \"RootElement::org::multipoly::User::User::usergroup\"");
				result.append(", \"to\": ");
				if ( baseClass.equals(UserGroup.class) ) {
					result.append(UserGroupRuntimePropertyEnum.asJson());
					result.append(", \"from\": ");
	...
	private void add(Map<Class<? extends UserGroup>,List<UmlgNodeJsonHolder>> resultMap, User parentResource, Map<String,Object> propertyMap) {
	...
	private void put(Map<Class<? extends UserGroup>,List<UmlgNodeJsonHolder>> resultMap, Map<String,Object> propertyMap) {
		Object id = propertyMap.get("id");
		UserGroup childResource = UMLG.get().getEntity(id);
